...
fig, axes = plt.subplots(1, 5, figsize=(7, 1.5),
...
for j, c in enumerate([1, 10]):
    ...
    axes[j].annotate(f'$c = {c}$', xy=(.6, .85), fontsize=6,
                     xycoords='axes fraction',
...
ent = ent + ent[::-1]
axes[1].plot(alpha, - ent, color='C0', label=r'$c = \infty$')
xs = np.linspace(-3, 3, 50)
axes[0].plot(xs, 1 / (1 + np.exp(-xs)), color='C0')

cbar = fig.colorbar(m, cax=axes[4], extend='both', orientation='vertical')
...
axes[0].annotate(r'$C =$', xy=(0.7, .45), xycoords='axes fraction',
                 ha='center', )
axes[0].annotate(r'$\begin{pmatrix}0 & c \\ c & 0\end{pmatrix}$',
                 xy=(0.7, .1), ha='center',
                 xycoords='axes fraction')
