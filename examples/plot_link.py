...
h, w = 32, 32
batch_size = 1
# alpha = torch.abs(torch.randn((batch_size, h, w)))
...
metric_space = _BaseGSpaceImpl(h, w, epsilon=2, sigma=3,
                               max_iter=1000,
                               tol=0,
                               method='lbfgs',
                               logspace=True,
...
alpha = alpha.to(device)
v, f = metric_space.entropy(alpha)
for i in range(1):
    t0 = time.perf_counter()
    v, proj = metric_space.lse(f, init=alpha)
    elapsed = time.perf_counter() - t0
    print(f'Time: {elapsed:.2f}')

sm = F.softmax(f.view(batch_size,  -1), dim=1).view(batch_size, h, w)
...
axes[0].annotate(r'Distribution $\alpha$', xy=(0.5, -0.1), ha='center', va='top',
                 xycoords='axes fraction'
                 )
axes[1].imshow(-f[0])
axes[1].annotate('Distance field \n' + r'$f = \nabla \Omega(\alpha)$', va='top', xy=(0.5, -0.1), ha='center',
                 xycoords='axes fraction'
                 )
axes[2].imshow(proj[0])
axes[2].annotate('Link function \n' + r'$\nabla \Omega^\star(f)$', xy=(0.5, -0.1), ha='center',
                 va='top',
                 xycoords='axes fraction'
                 )
axes[3].imshow(sm[0])
axes[3].annotate('Naive \n' + r'$\text{softmax}(f)$', xy=(0.5, -0.1), ha='center',
                 va='top',
                 xycoords='axes fraction')
for ax in axes:
    ax.axis('off')
